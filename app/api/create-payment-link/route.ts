...
async function preparePaymentSessionData(data: RequestPayload) {
  ...
  if (customSlotTypes?.length > 0 && slotDetails?.length > 0) {
    const groupedSlots = slotDetails.reduce(
      ...
      {} as Record<
        string,
        { count: number; price: number; customFields: Record<string, any> }
      >
    );
  ...
  const mappedTourProducts: LineItemInput[] = tourProducts.map((p: any) => ({
    label: p.name,
  ...
  console.log("mappedTourProducts", mappedTourProducts);

  let allItems: LineItemInput[] = [];

  if (tourProducts.length !== 0) {
    allItems = [...normalizedSlots, ...mappedTourProducts];
  } else {
    allItems = normalizedSlots;
  }
